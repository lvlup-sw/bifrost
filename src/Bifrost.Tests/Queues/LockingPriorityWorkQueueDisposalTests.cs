// =============================================================================
// <copyright file="LockingPriorityWorkQueueDisposalTests.cs" company="Levelup Software">
...
/// <summary>
/// Disposal tests for <c>LockingPriorityWorkQueue&lt;TWork&gt;</c> (DR-3, issue #21):
/// the exact-ordering lock-based priority binding owns an item-counting
/// <see cref="SemaphoreSlim"/> wake-up, an <see cref="IDisposable"/> resource that must
/// be released. Before DR-3 the type did not implement <see cref="IDisposable"/> —
/// leaking the semaphore (a CA2213-class undisposed-field defect) — so these tests pin
/// the new <see cref="IDisposable"/> contract: <c>Dispose</c> disposes the semaphore
/// (observed via a subsequent wait surfacing <see cref="ObjectDisposedException"/>) and
/// is idempotent. Mirrors <see cref="ConcurrentPriorityWorkQueueDisposalTests"/>.
/// </summary>
public sealed class LockingPriorityWorkQueueDisposalTests
{
    /// <summary>
    ...
    [Test]
    public async Task LockingPriorityWorkQueue_Dispose_ThenWait_ThrowsObjectDisposedException()
    {
        // Arrange — a non-completed, empty queue: a wait would park on the semaphore.
        var queue = new LockingPriorityWorkQueue<int>(
    ...
    /// <returns>A task representing the asynchronous test.</returns>
    [Test]
    public async Task LockingPriorityWorkQueue_DisposeTwice_DoesNotThrow()
    {
        // Arrange
        var queue = new LockingPriorityWorkQueue<int>(
            SmallCapacity, new PriorityDispatchOptions(), TestTimestampFrequency);
