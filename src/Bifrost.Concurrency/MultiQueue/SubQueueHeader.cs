...
// </copyright>
// =============================================================================
// Ported from lvlup-sw/DataFerry@2bf0456 (src/DataFerry/Concurrency/MultiQueue/SubQueueHeader.cs)
...
/// <remarks>
/// <para>
/// <b>Why this type exists.</b> In the two-choice MultiQueue design, threads sample two random
...
/// them, two hot fields sharing a cache line would cause <b>false sharing</b>: a write to one
...
/// </para>
/// <para>
/// <b>Why 128 bytes and not 64.</b> This mirrors the BCL's <c>PaddedHeadAndTail</c> pattern used
...
/// </para>
/// <para>
/// <b>Why explicit layout, and what is missing.</b> <see cref="StructLayoutAttribute"/> with
/// <see cref="LayoutKind.Explicit"/> is the only way to pin fields to exact byte offsets, but the
/// CLR <b>forbids explicit layout on generic types</b>. The cached top <i>priority</i> is a
/// generic <c>TPriority</c>, so it cannot live here. It is stored separately by
/// <see cref="PaddedTopSlot{TPriority}"/>, which achieves the same cache-line isolation using a
/// padded array instead of explicit offsets. This struct therefore holds only the non-generic
/// hot words; the generic top is isolated elsewhere.
/// </para>
/// <para>
/// <b>Cold fields live elsewhere.</b> The heap array reference and heap size are only ever touched
/// under the sub-queue lock, are never sampled lock-free, and so deliberately do <b>not</b> live
/// in this padded header. Keeping them out avoids wasting cache lines on fields that never
/// contend.
/// </para>
/// <para>
