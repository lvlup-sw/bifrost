...
/// test caught it. These three tests exist to make that failure mode impossible to ship silently:
/// they were each validated against a deliberately inverted (max-heap) build via the kill-probe
/// recorded in the task notes — every one fails decisively (observed mean rank near N/2 ≈ 5000
/// versus the bound ≈ 27 for n=16) — and pass against the real min-heap build.
/// </para>
/// <para>
...
/// </para>
/// <para>
/// <b>Measured duration.</b> On the development machine the full three-test class completes in
...
public class RankErrorTests
{
    ...
    /// relaxation roughly linearly in <c>s</c> (so the looseness now compounds across both the
    ...
    /// <summary>
    /// The mean/P99 quality bound (DR-16): a full single-threaded drain's empirical mean rank must
    /// sit within 2× the <c>(5/6)·n·s</c> theoretical mean and its P99 within <c>10·n·s</c>, at
    /// every pinned <c>(n, s)</c> combination.
    /// </summary>
    /// <param name="subQueueCount">The exact sub-queue count to pin.</param>
    /// <param name="stickiness">The stickiness factor <c>s</c> to pin.</param>
    [Test]
    [Arguments(4, 1)]
    [Arguments(16, 1)]
    [Arguments(16, 4)]
    ...
    /// <summary>
    /// The stickiness-sensitivity proof: at a fixed sub-queue count the empirical mean rank under
    /// <c>s=4</c> must strictly exceed that under <c>s=1</c>, so a dequeue that silently ignored
    /// <c>s</c> would be caught.
    /// </summary>
    [Test]
    ...
    /// <summary>
    /// The explicit anti-F-7 inversion alarm, verified across stickiness levels: fewer than 1% of
    /// pops may land in the top decile of the remaining population — an inverted (max-heap) build
    /// puts essentially every pop there.
    /// </summary>
    /// <param name="stickiness">The stickiness factor <c>s</c> to pin.</param>
    [Test]
    [Arguments(1)]
    ...
    /// <summary>
    /// The distribution shape check (pinned at <c>s=1</c>): rank mass must concentrate near zero
    /// and decay strictly across the first four <c>n</c>-wide buckets — an inverted build puts
    /// essentially no mass in those low buckets at all.
    /// </summary>
    [Test]
    ...
    /// <summary>
    /// The single shared drain used by all three tests so they cannot diverge on how ranks are
    ...
    private sealed class BinaryIndexedTree
    {
        ...
        {
            for (int i = priority + 1; i <= this.length; i += i & -i)
            {
                this.tree[i] += delta;
            }
        }
