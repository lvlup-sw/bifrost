...
Throughput, single-thread latency, and stickiness measurements for the Bifrost `ConcurrentPriorityQueue` on a 64-thread (32 physical-core) Xeon. The relaxed queue is measured against a Naive single-lock priority queue (one global lock around a binary heap) and against its own strict `TryDequeueMin` scan path, across four workloads.
