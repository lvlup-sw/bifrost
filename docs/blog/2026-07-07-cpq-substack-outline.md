...
PQs have a lot of interesting properties. Unlike a standard FIFO queue or stack, arrival order *does not matter* and high-priority items added last will still come out first. This sounds like a straightforward principle, but it functionally means that you must keep strict global order by comparing priorities, which means sorting, which means either time or space complexity as a tradeoff. For instance, in a sorted Array or List, extracting the highest priority element is instant O(1) time since it's always sitting at the "front". Insertion, however, is quite slow; you have to scan the entire collection to find the right spot, and then shift all subsequent items to make room -- linear O(n) time.

For these reasons, PQs are very typically implemented as binary heaps. In this data structure, the *root* of a heap is always the highest priority item, so we preserve the O(1) time for explicit lookups (peek). Because a binary heap is a tree-structure, and it stays perfectly balanced, we know its height is always $\log_2(n)$. Thus, the maximum distance from top to bottom is $\log n$ which nets us $O(\log n)$ insertion time -- a significant improvement. This does mean we also have to rebalance the collection on delete-min operations, which means we also degrade to $O(\log n)$ for those operations. Overall this is considered worth the trade-off.
