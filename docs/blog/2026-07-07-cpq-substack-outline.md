...
This struck me as genuinely strange when I stumbled onto it over four years ago, during the pandemic (man, time flies). Why aren't there any CPQs *anywhere*?! Surely it couldn't be *that* hard to implement, right? I mean, it's not like a priority queue runs headfirst into the one property that makes concurrency scale in the first place. Reader, it does exactly that. A stack of research papers and one intensely personal relationship with the ARM memory model later, I can confirm: it was that hard.
